...
8. purpose는 결제 목적과 상세 사유를 50자 이내로 작성하세요.
...
def load_data() -> pd.DataFrame:
    try:
        sb = get_supabase()
        res = sb.table("receipts").select("*").order("id", desc=False).execute()
...
def delete_row(row_id: int):
    sb = get_supabase()
    sb.table("receipts").delete().eq("id", row_id).execute()


def bulk_insert(rows: list):
    sb = get_supabase()
    for r in rows:
        r.pop("id", None)
    sb.table("receipts").insert(rows).execute()
...
def import_excel(file, author: str) -> int:
    EXCEL_COL_MAP = {
        "사용일자": "date", "사용시간": "time", "결제지역(시)": "location",
        "가맹점명": "merchant", "결제금액(원)": "amount", "계정과목(분류)": "category",
        "목적/상세사유": "purpose", "참석자/대상처": "attendees",
    }
    xl = pd.read_excel(file).rename(columns=EXCEL_COL_MAP)
    rows = []
    for _, row in xl.iterrows():
        ...
        time_str = time_raw[:5] if pd.notna(row.get("time")) and time_raw != "nan" else ""
        rows.append({
            "author": author,
            "date": date_str,
            "time": time_str,
...
def page_register():
    ...
    with tab2:
        ...
        st.caption("컬럼: 사용일자, 사용시간, 결제지역(시), 가맹점명, 결제금액(원), 계정과목(분류), 목적/상세사유, 참석자/대상처")
    ...
    with tab1:
        ...
        with st.form("receipt_form"):
            ...
            with col5:
                amount_input = st.number_input("결제금액(원)", min_value=0, step=100, value=int(extracted.get("amount", 0)))
            with col6:
            ...
            purpose_input = st.text_input("목적/상세사유", value=extracted.get("purpose", ""), placeholder="예: 외부 미팅 후 식사")
            attendees_input = st.text_input("참석자/대상처", value=extracted.get("attendees", ""), placeholder="예: 대표님, 권혁제 / 이지의원")
...
def page_dashboard():
    ...
    with col_a:
        ...
        fig_author = px.pie(by_author, values="amount", names="author",
    ...
    with col_b:
        ...
        fig_cat = px.bar(by_cat, x="category", y="amount", color="category",
    ...
    fig_month = px.line(by_month, x="year_month", y="amount", markers=True,
    ...
    col_labels = {"id": "No", "author": "작성자", "date": "사용일자", "time": "시간",
                  "merchant": "가맹점명", "location": "결제지역", "amount": "결제금액",
                  "category": "계정과목", "purpose": "목적/사유", "attendees": "참석자/대상처"}
    st.dataframe(display_df[show_cols].rename(columns=col_labels), use_container_width=True, hide_index=True)
    ...
    selected_label = st.selectbox("삭제할 영수증 선택", options=list(delete_options.keys()),
...
# ── 메인 ──────────────────────────────────────────────────────────────────────
def main():
    st.set_page_config(page_title="법인카드 영수증 관리", page_icon="🧾", layout="wide")

    with st.sidebar:
        st.title("🧾 법인카드 관리")
        st.caption("영수증 지출 관리 시스템")
        st.divider()
        menu = st.radio("메뉴", ["📄 영수증 등록", "📊 지출 대시보드"], label_visibility="collapsed")
